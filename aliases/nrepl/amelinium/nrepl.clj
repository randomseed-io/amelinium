(ns ^{:clojure.tools.namespace.repl/load false} amelinium.nrepl
  (:require
...
(defn- configured-port
  ^Long []
  (or (parse-long* (System/getProperty "phone-number.nrepl.port"))
      (parse-long* (System/getProperty "nrepl.port"))
      (parse-long* (System/getenv "phone-number_NREPL_PORT"))
      (parse-long* (System/getenv "NREPL_PORT"))
      default-port))

(defn- configured-port-file
  ^String []
  (or (not-empty (System/getProperty "phone-number.nrepl.port-file"))
  ...
  ([{:keys [port port-file]
   ...
   (or @!server
       (let [server (start-server :port (long port) :handler (nrepl-handler))
         ...
         (println (str "[phone-number] nREPL client can be connected to port " (:port server)
                       " (" (.getPath pf) ")"))
         (reset! !server server)))))
...
(println "[phone-number] Starting nREPL server")
